...
class VKDataSource extends DataSource {
  ...
  Future<List<Post>> getByKeyword(String keyword) async {
    ...
    if (token != '') {
      try {
        final response = await _dio.get<Map<String, dynamic>>('https://api.vk.com/method/groups.search?access_token=$token&q=$keyword&count=5&v=5.131&sort=6');

        final result = response.data;
        // print(result);
        var groups = result!['response']['items'];
        for (var group in groups) {
          var groupPostsResponse = await _dio.get<Map<String, dynamic>>('https://api.vk.com/method/wall.get?access_token=$token&owner_id=-${group['id']}&count=2&v=5.131');
              ...
              for (var post in groupPosts) {
                ...
                for (final attachment in post['attachments']) {
                  if (attachment is Map &&
                      (attachment.containsKey('audio') || attachment.containsKey('video') || attachment.containsKey('doc') || attachment.containsKey('graffiti') || attachment.containsKey('link') ||
                          attachment.containsKey('note') || attachment.containsKey('app') || attachment.containsKey('poll') || attachment.containsKey('page') || attachment.containsKey('album'))) {
                    isRelink = true;
                  }
                }
                posts.add(Post(
